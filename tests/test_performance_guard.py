"""The performance-envelope guard (``Kinematics.validate_performance``).
...
Each kinematics rejects an envelope it cannot fly, both at :class:`~opencdarr.fleet.Agent`
construction (the explicit case, failing at the line the mistake is written) and again at the
composition root (the ``kinematics=None`` default case, whose effective model is not known
earlier).
"""
...
def test_matched_envelopes_construct() -> None:
    """The two built-in airframes pair with their own kinematics without complaint."""
...
def test_default_kinematics_mismatch_caught_at_composition_root() -> None:
    """``kinematics=None`` defers to the Multirotor default, so the mismatch surfaces in
    run_fleet."""
    intr = create_conflict(_OWN, intr_id="INTR", dpsi=90.0, dcpa=0.0,
                           tlos=30.0, rpz=50.0, gs_intr=15.0, side=1)
    # Agent construction is fine — the effective kinematics is not known yet.
