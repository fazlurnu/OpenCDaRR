...
class Comm(CommunicationModel):
    ...
    def _offer(
        ...
        Per directed link: draw reception, then — **only if received** — latency. That order is
        what ADR 0006 §6 pins, so a dropped message costs one draw and a delivered one costs two
        (or one, since :func:`constant_latency` draws nothing). A drop enqueues nothing at all,
        which is exactly what leaves the receiver holding whatever it already had.
        """
    ...
    def step(
        ...
        # the two halves of a tick, and the only two that consume anything: what the links accept
        # onto the air, then what has arrived by now. All the randomness is in the first.
        in_flight = self._offer(state.in_flight, broadcasts, receivers, rng)
        held, still_flying = self._deliver(state.held, in_flight, t)
        return CommState(held=held, in_flight=still_flying)
...
class RadioState(CommState):
    """:class:`CommState` plus each aircraft's radio health — :class:`TransceiverComm`'s state.

    ``tx_down`` / ``rx_down`` are the ids whose **transmitter** / **receiver** is currently failed.
    ...
    *receiver* has failed is flying blind while its transmitter keeps squittering, so the rest of
    ...
    which is why :meth:`~opencdarr.cns.base.CommunicationModel.initial_state` needs no roster.

    ``t_prev`` is when :meth:`TransceiverComm.step` last ran (``None`` before the first call). The
    hazard is applied over the *elapsed* time rather than per call, so offset broadcast phases and
    jitter — which make the gap between calls something other than the nominal interval — come out
    right without the model being told the cadence at all.
    """
...
class TransceiverComm(Comm):
    """:class:`Comm` whose per-aircraft transmitter and receiver fail — and recover — on their own.
    ...
    it. This model adds that: each subsystem of each aircraft fails at ``tx_fail_rate`` /
    ``rx_fail_rate`` and comes back at ``tx_recover_rate`` / ``rx_recover_rate``, all in **events
    per second** (mean time to failure = ``1 / rate``). Reception and latency are inherited
    unchanged and apply to whatever the working radios still carry, so the two effects compose.
    ...
    (``TransceiverComm(rx_fail_rate=1e-3)`` fails receivers and nothing else).
    ...
    **holding** what it last received (:class:`~opencdarr.cns.surveillance.LastKnown`) and decides
    ...
    is why an outage is not the same experiment as ``reception_prob=0``.

    The gate is applied when a broadcast is **offered**. A message the link already accepted stays
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(reception_prob=reception_prob, latency=latency)
        rates = {
        ...
        self.rx_fail_rate = rx_fail_rate
        self.tx_recover_rate = tx_recover_rate
        self.rx_recover_rate = rx_recover_rate

    def initial_state(self) -> RadioState:
        """Every radio working, nothing delivered, no elapsed time yet."""
        return RadioState()
    ...
    def step(
        ...
        if not isinstance(state, RadioState):
            raise TypeError(
                f"{type(self).__name__} threads its own RadioState, got "
                f"{type(state).__name__}. It comes from initial_state() — build the run through "
                "CnsState.initial(n, model) (which run_fleet does) rather than handing this model "
                "a bare CommState."
            )

        # 1. evolve every radio first, at a fixed offset from the start of the step, so the outage
        #    sequence does not shift with how the reception draws happened to fall. No time has
        #    elapsed on the first call, so nothing can have failed at t = 0.
        elapsed = 0.0 if state.t_prev is None else max(0.0, t - state.t_prev)
        tx_down, rx_down = set(state.tx_down), set(state.rx_down)
        ...
        # 2. a silent transmitter offers nothing; a deaf receiver is offered nothing. Both keep
        #    holding what they already had, which is the point — they fly on ageing data.
        base = super().step(
            state,
            [m for m in broadcasts if m.source not in tx_down],
            [r for r in receivers if r not in rx_down],
            t,
            rng,
        )
        return RadioState(
            ...
            tx_down=frozenset(tx_down),
            rx_down=frozenset(rx_down),
            t_prev=t,
        )
