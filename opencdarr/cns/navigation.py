...
class GpsNavigation(NavigationModel):
    ...
    ``pos_ci95`` — 95% radial position accuracy [m]; ``vel_std`` — per-axis velocity error
    [m/s]; ``distribution`` — the position-error model (default isotropic Gaussian).
    """
    ...
    def measure(self, true: AircraftState, t: float, rng: np.random.Generator) -> Message:
        ...
        ve += float(rng.normal(0.0, self.vel_std))
        vn += float(rng.normal(0.0, self.vel_std))
